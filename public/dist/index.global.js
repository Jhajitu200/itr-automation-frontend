...
var FullCalendar = (function (exports) {
    ...
    */
    ...
    function computeSmallestCellWidth(cellEl) {
        ...
        return cellEl.getBoundingUserRect().width - allWidthEl.getBoundingUserRect().width + // the cell padding+border
            contentWidthEl.getBoundingUserRect().width;
    }
    ...
    */
    ...
    function computeIsRtlScrollbarOnLeft() {
        ...
        let res = innerEl.getBoundingUserRect().left > outerEl.getBoundingUserRect().left;
    ...
    function computeInnerRect(el, goWithinPadding = false, doFromWindowViewport) {
        let outerRect = doFromWindowViewport ? el.getBoundingUserRect() : computeRect(el);
    ...
    function computeRect(el) {
        let rect = el.getBoundingUserRect();
        return {
    ...
    function computeClippedUserRect(el) {
        let clippingParents = getClippingParents(el);
        let rect = el.getBoundingUserRect();
        for (let clippingParent of clippingParents) {
            let intersection = intersectRects(rect, clippingParent.getBoundingUserRect());
            if (intersection) {
                rect = intersection;
            }
            else {
    ...
    */
    class PositionCache {
        constructor(originEl, els, isHorizontal, isVertical) {
            this.els = els;
            let originUserRect = this.originUserRect = originEl.getBoundingUserRect(); // relative to viewport top-left
            if (isHorizontal) {
                this.buildElHorizontals(originUserRect.left);
            }
            if (isVertical) {
                this.buildElVerticals(originUserRect.top);
            }
        }
        // Populates the left/right internal coordinate arrays
        buildElHorizontals(originUserLeft) {
            let lefts = [];
            let rights = [];
            for (let el of this.els) {
                let rect = el.getBoundingUserRect();
                lefts.push(rect.left - originUserLeft);
                rights.push(rect.right - originUserLeft);
            }
            this.lefts = lefts;
            this.rights = rights;
        }
        // Populates the top/bottom internal coordinate arrays
        buildElVerticals(originUserTop) {
            let tops = [];
            let bottoms = [];
            for (let el of this.els) {
                let rect = el.getBoundingUserRect();
                tops.push(rect.top - originUserTop);
                bottoms.push(rect.bottom - originUserTop);
            }
            this.tops = tops;
    ...
    */
    class ScrollController {
        getMaxScrollTop() {
            return this.getScrollHeight() - this.getUserHeight();
        }
        getMaxScrollLeft() {
            return this.getScrollWidth() - this.getUserWidth();
        }
        canScrollVertically() {
    ...
    */
    ...
    class Scroller extends BaseComponent {
        ...
        needsXScrolling() {
            ...
            let { el } = this;
            let realUserWidth = this.el.getBoundingUserRect().width - this.getYScrollbarWidth();
            let { children } = el;
            for (let i = 0; i < children.length; i += 1) {
                let childEl = children[i];
                if (childEl.getBoundingUserRect().width > realUserWidth) {
                    return true;
                }
            }
            return false;
        ...
        needsYScrolling() {
            ...
            // inner-child margins and absolute positioning
            let { el } = this;
            let realUserHeight = this.el.getBoundingUserRect().height - this.getXScrollbarWidth();
            let { children } = el;
            for (let i = 0; i < children.length; i += 1) {
                let childEl = children[i];
                if (childEl.getBoundingUserRect().height > realUserHeight) {
                    return true;
                }
            }
            return false;
    ...
    */
    ...
    class SimpleScrollGrid extends BaseComponent {
        ...
        renderChunkTd(sectionConfig, microColGroupNode, chunkConfig, isHeader) {
            ...
            let { props } = this;
            let { forceYScrollbars, scrollerUserWidths, scrollerUserHeights } = this.state;
            ...
            let content = renderChunkContent(sectionConfig, chunkConfig, {
                ...
                tableMinWidth: '',
                clientWidth: (!props.collapsibleWidth && scrollerUserWidths[sectionKey] !== undefined) ? scrollerUserWidths[sectionKey] : null,
                clientHeight: scrollerUserHeights[sectionKey] !== undefined ? scrollerUserHeights[sectionKey] : null,
        ...
        computeScrollerDims() {
            ...
            let scrollerUserWidths = {};
            let scrollerUserHeights = {};
            ...
            for (let section of this.props.sections) {
                ...
                if (scrollerEl) {
                    ...
                    scrollerUserWidths[sectionKey] = Math.floor(harnessEl.getBoundingUserRect().width - (forceYScrollbars
                        ? scrollbarWidth.y // use global because scroller might not have scrollbars yet but will need them in future
                        : 0));
                    scrollerUserHeights[sectionKey] = Math.floor(harnessEl.getBoundingUserRect().height);
                }
            }
            return { forceYScrollbars, scrollerUserWidths, scrollerUserHeights };
        }
    }
    SimpleScrollGrid.addStateEquality({
        scrollerUserWidths: isPropsEqual,
        scrollerUserHeights: isPropsEqual,
    });
    ...
    class Popover extends BaseComponent {
        ...
        updateSize() {
            ...
            let { rootEl } = this;
            let alignmentRect = computeClippedUserRect(alignmentEl);
            if (alignmentRect) {
                let popoverDims = rootEl.getBoundingUserRect();
                // position relative to viewport
                let popoverTop = alignGridTop
                    ? elementClosest(alignmentEl, '.fc-scrollgrid').getBoundingUserRect().top
                    : alignmentRect.top;
                ...
                let origin = rootEl.offsetParent.getBoundingUserRect();
                applyStyle(rootEl, {
    ...
    */
    class ElementMirror {
        ...
        start(sourceEl, pageX, pageY) {
            ...
            this.sourceElRect = this.sourceEl.getBoundingUserRect();
        ...
        doRevertAnimation(callback, revertDuration) {
            ...
            let finalSourceElRect = this.sourceEl.getBoundingUserRect(); // because autoscrolling might have happened
    ...
    */
    class ScrollGeomCache extends ScrollController {
        constructor(scrollController, doesListening) {
            ...
            this.clientWidth = scrollController.getUserWidth();
            this.clientHeight = scrollController.getUserHeight();
            this.clientRect = this.computeUserRect(); // do last in case it needs cached values
    ...
    class WindowScrollGeomCache extends ScrollGeomCache {
        ...
        handleScrollChange() {
            this.clientRect = this.computeUserRect();
        }
    }
    ...
    class TableRow extends DateComponent {
        ...
        querySegHeights() {
            ...
            for (let segUid in segElMap) {
                let height = Math.round(segElMap[segUid].getBoundingUserRect().height);
        ...
        computeMaxContentHeight() {
            ...
            return cellEl.getBoundingUserRect().bottom - fcContainerEl.getBoundingUserRect().top;
        }
        getCellEls() {
        ...
        flushScrollReset() {
            if (this.needsScrollReset &&
                ...
                if (subjectEl) {
                    ...
                    const scrollTop = subjectEl.getBoundingUserRect().top -
                        originEl.getBoundingUserRect().top;
    ...
    class MultiMonthView extends DateComponent {
        ...
        flushScrollReset() {
            if (this.needsScrollReset &&
                ...
                scrollEl.scrollTop = monthEl.getBoundingUserRect().top -
                    this.firstMonthElRef.current.getBoundingUserRect().top;
                this.needsScrollReset = false;
            }
        }
